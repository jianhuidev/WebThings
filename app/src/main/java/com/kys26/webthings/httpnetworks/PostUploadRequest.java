...
public class PostUploadRequest extends Request<JSONObject> {
    ...
    public PostUploadRequest(Context mContext, String url, Map<String, String[]> fileMap, Response.Listener<JSONObject> mListener, Response.ErrorListener listener) {
        super(Method.POST, url, listener);
        this.context = mContext;
        this.mListener = mListener;
        this.fileMap = fileMap;
        MethodTools.sPreFerCookie = mContext.getSharedPreferences("Cookies",
                Context.MODE_PRIVATE);
        mHeader.put("Cookie", MethodTools.sPreFerCookie.getString("cookie", "null"));
    ...
    protected Response<JSONObject> parseNetworkResponse(NetworkResponse response) {
        try {
            ...
            Log.e("请求", "respone:" + response.toString() + "responeCode:" + response.statusCode);
    ...
    private void buildTextPart(DataOutputStream ds) {
        try {
            ...
            for (Object name : key_arr) {
                ...
                ds.write(("Content-Type: application/json;charset=UTF-8").getBytes("UTF-8"));
                ds.write((end).getBytes("UTF-8"));
